...
//
// Component wrapper yang:
//   1. Menjalankan device validation via useDevice
//   2. Redirect berdasarkan device status — dengan logika platform-aware
//   3. Desktop/laptop → bypass physical binding, 'not_found' TIDAK diblok
//   4. TC21/mobile production → wajib binding, 'not_found' redirect ke /device-bind
//
// ── ARSITEKTUR: SPLIT INNER COMPONENT ────────────────────────────────────────
//
//   MASALAH sebelumnya:
//     useDevice() dipanggil di DeviceGuard tanpa kondisi, meskipun pathname
//     adalah /device-bind. React tidak mengizinkan hook dipanggil kondisional,
//     jadi tidak bisa "if (isExemptPath) skip useDevice()".
//
//     Akibatnya: initDevice() + server fetch tetap jalan di /device-bind →
//     hydration race → isLoading = true → loading screen render → page stuck.
//
//   SOLUSI:
//     Split menjadi dua inner component:
//
//     1. <DeviceGuardExempt>   — untuk exempt path (/device-bind, dll)
//        Tidak memanggil useDevice() sama sekali.
//        Langsung render children tanpa guard apapun.
//        Zero server fetch, zero loading state.
//
//     2. <DeviceGuardActive>   — untuk semua route lain
//        Memanggil useDevice() dan menjalankan semua guard logic.
//        Redirect TC21 belum bound → /device-bind.
//        Redirect blocked/inactive → halaman error.
//
//     DeviceGuard (root) hanya bertugas memilih mana yang dirender
//     berdasarkan pathname — SEBELUM hook apapun dipanggil.
//
// ── REDIRECT FLOW FINAL ───────────────────────────────────────────────────────
//
//   TC21 (belum bind):
//     open app → DeviceGuardActive: Android=true, isDeviceBound()=false
//     → redirect /device-bind → DeviceGuardExempt → render langsung ✓
//     → scan QR → bind sukses → redirect / → DeviceGuardActive aktif lagi
//     → useDevice fetch → registered → pass → login → /home ✓
//
//   TC21 (sudah bind):
//     open app → DeviceGuardActive: isDeviceBound()=true → pass
//     → useDevice fetch → registered → render ✓
//
//   Desktop admin:
//     open app → DeviceGuardActive: isAndroid=false → skip binding check
//     → useDevice fetch → registered/not_found (allow) → render ✓
//
// ── ATURAN REDIRECT ──────────────────────────────────────────────────────────
//   Desktop:  not_found → lewat saja
//             blocked   → /device-blocked
//             inactive  → /device-inactive
//   TC21:     not_found + belum bound → /device-bind
//             not_found + sudah bound → null (tunggu re-check)
//             blocked   → /device-blocked
//             inactive  → /device-inactive
